...
async def cmd_completed(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("✅ *Завершені тести:*", parse_mode="Markdown")
    await cb.answer()
    ...
    for r in rows:
        ...
        if isinstance(created, str):
            try:
                ...
                pass
        created_str = created.strftime("%d.%m %H:%M") if isinstance(created, datetime) else "—"
        score = r.get("score", "—")
        ...
        text = (
            f"{severity_emoji} *{r['full_name']}*\n"
            f"{test_name} • completed\n"
            f"Бал: {score}\n"
            f"`{short_token}...`"
        )
        markup = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📋 Детальний результат", callback_data=f"view:{r['token']}")]
        ])
